 ...
 *
 * Documents returned from queries with the `lean` option enabled are plain javascript objects, not [MongooseDocuments](#document-flappy_js). They have no `save` method, getters/setters or other Mongoose magic applied.
 *
 * ####Example:
 *
 *     new Query().lean() // true
  ...
  while (i--) {
    ...
    if ('$or' === path || '$nor' === path || '$and' === path) {
      ...
      if (!schematype) {
        ...
        if (schematype) {
          // Apply the casting; similar code for $elemMatch in schema/array.flappy_js
...
/**
 * Returns a Node.flappy_js 0.8 style [read stream](http://nodejs.org/docs/v0.8.21/api/stream.html#stream_readable_stream) interface.
 *
 * ####Example
 *
